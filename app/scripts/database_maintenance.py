...
async def archive_stats():
    ...
    try:
        ...
        if stats['active_date_range']['oldest']:
            print(f"Active data range: {stats['active_date_range']['oldest']} to {stats['active_date_range']['newest']}")
        
        if stats['archive_date_range']['oldest']:
            print(f"Archive data range: {stats['archive_date_range']['oldest']} to {stats['archive_date_range']['newest']}")
...
def main():
    """Main CLI function"""
    parser = argparse.ArgumentParser(description="NFL Sentiment Analyzer Database Maintenance")
    ...
    subparsers.add_parser('archive-maintenance', help='Run complete archiving maintenance')
