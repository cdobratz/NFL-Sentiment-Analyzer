...
from ..services.scheduled_tasks_service import get_scheduled_tasks_service, ScheduledTasksService
...
async def get_analytics(
    ...
    try:
        ...
        async for doc in db.performance_metrics.aggregate(model_performance_pipeline):
            model_performance.append({
                ...
                "avg_prediction_time_ms": round(doc.get("avg_prediction_time", 0), 2),
    ...
    try:
        error_stats_pipeline = [
            {"$match": {"timestamp": {"$gte": start_date}, "level": {"$in": ["ERROR", "CRITICAL"]}}},
            {"$group": {
                "_id": "$error_type",
                "count": {"$sum": 1},
...
async def get_model_performance(
    ...
    try:
        ...
        if metrics:
            avg_accuracy = sum(m.get("accuracy", 0) for m in metrics if m.get("accuracy")) / len([m for m in metrics if m.get("accuracy")])
            avg_f1_score = sum(m.get("f1_score", 0) for m in metrics if m.get("f1_score")) / len([m for m in metrics if m.get("f1_score")])
...
async def get_system_alerts(
    ...
    try:
        ...
        critical_alerts = await db.alerts.count_documents({**query, "severity": "critical"})
...
async def clear_cache(
    cache_type: Optional[str] = Query(None, description="Type of cache to clear (redis, all)"),
    current_admin: dict = Depends(get_current_admin_user),
    redis = Depends(get_redis)
):
