...
class HealthChecker:
    ...
    async def check_ml_services(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result = await sentiment_service.analyze_text("Test sentiment analysis")
...
async def detailed_health_check():
    ...
    mongodb_check, redis_check, external_apis_check, ml_services_check = await asyncio.gather(
    ...
    result = {
        ...
        "components": {
            "mongodb": mongodb_check if not isinstance(mongodb_check, Exception) else {"status": "error", "error": str(mongodb_check)},
            "redis": redis_check if not isinstance(redis_check, Exception) else {"status": "error", "error": str(redis_check)},
            "external_apis": external_apis_check if not isinstance(external_apis_check, Exception) else {"status": "error", "error": str(external_apis_check)},
            "ml_services": ml_services_check if not isinstance(ml_services_check, Exception) else {"status": "error", "error": str(ml_services_check)}
        },
...
async def readiness_check():
    ...
    try:
        ...
        if mongodb_status.get("status") != "healthy" or redis_status.get("status") != "healthy":
...
async def metrics_endpoint():
    ...
    if "cpu" in system_metrics:
        metrics.append(f"nfl_analyzer_cpu_usage_percent {system_metrics['cpu']['usage_percent']}")
        metrics.append(f"nfl_analyzer_cpu_count {system_metrics['cpu']['count']}")
    
    if "memory" in system_metrics:
        metrics.append(f"nfl_analyzer_memory_total_bytes {system_metrics['memory']['total']}")
        metrics.append(f"nfl_analyzer_memory_used_bytes {system_metrics['memory']['used']}")
        metrics.append(f"nfl_analyzer_memory_usage_percent {system_metrics['memory']['percent']}")
    
    if "disk" in system_metrics:
        metrics.append(f"nfl_analyzer_disk_total_bytes {system_metrics['disk']['total']}")
        metrics.append(f"nfl_analyzer_disk_used_bytes {system_metrics['disk']['used']}")
        metrics.append(f"nfl_analyzer_disk_usage_percent {system_metrics['disk']['percent']}")
