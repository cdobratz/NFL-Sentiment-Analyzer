...
async def analyze_sentiment(
    ...
    auth: Union[dict, APIKey] = Depends(require_user_or_api_scope(APIKeyScope.WRITE_SENTIMENT))
):
    ...
    try:
        ...
        background_tasks.add_task(
            ...
            db, sentiment_data.team_id, sentiment_data.player_id, sentiment_data.game_id
        )
...
async def analyze_batch_sentiment(
    ...
    try:
        ...
        aggregated_sentiment = sum(r.sentiment_score for r in results) / len(results) if results else 0.0
        
        sentiment_distribution = {
            SentimentLabel.POSITIVE: sum(1 for r in results if r.sentiment == SentimentLabel.POSITIVE),
            SentimentLabel.NEGATIVE: sum(1 for r in results if r.sentiment == SentimentLabel.NEGATIVE),
            SentimentLabel.NEUTRAL: sum(1 for r in results if r.sentiment == SentimentLabel.NEUTRAL)
        }
        
        # Update aggregated sentiment in background
        background_tasks.add_task(
            update_aggregated_sentiment,
            db, batch_request.team_id, batch_request.player_id, batch_request.game_id
        )
...
async def get_team_sentiment(
    ...
    sources: Optional[List[DataSource]] = Query(None, description="Filter by data sources"),
    categories: Optional[List[SentimentCategory]] = Query(None, description="Filter by categories"),
    db = Depends(get_database)
):
...
async def get_game_sentiment(
    ...
    try:
        ...
        return GameSentiment(
            ...
            sentiment_by_category=sentiment_by_category,
            home_team_sentiment=home_sentiment_data[0]["avg_sentiment"] if home_sentiment_data else 0.0,
            away_team_sentiment=away_sentiment_data[0]["avg_sentiment"] if away_sentiment_data else 0.0,
...
async def get_recent_sentiments(
    limit: int = Query(10, ge=1, le=100, description="Number of recent analyses to return"),
    team_id: Optional[str] = None,
    player_id: Optional[str] = None,
    db = Depends(get_database)
):
...
async def aggregate_sentiment(
    request: SentimentAggregationRequest,
    db = Depends(get_database)
):
    ...
    try:
        ...
        if request.include_trends:
            trend_format = "%Y-%m-%d-%H" if request.trend_interval == "hour" else "%Y-%m-%d"
            trend_pipeline = [
                {"$match": query},
                {"$group": {
                    "_id": {
...
async def get_sentiment_insights(
    ...
    try:
        ...
        key_themes = sorted(category_counts.keys(), key=lambda x: category_counts[x], reverse=True)[:5]
        ...
        sentiment_drivers = dict(sorted(
            sentiment_drivers.items(), 
        ...
        if avg_sentiment < -0.3:
            ...
            recommendations.append("Leverage positive sentiment for marketing opportunities")
        else:
