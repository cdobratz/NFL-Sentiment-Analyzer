...
class SentimentAnalysisService:
    ...
    async def analyze_text(
        ...
        # Extract NFL context
        nfl_context = self._extract_nfl_context(processed_text, team_id, player_id, game_id)
        
        # Determine category
        category = self._categorize_text(processed_text)
        
        # Perform sentiment analysis
        sentiment_score, confidence = self._analyze_sentiment(processed_text, nfl_context, category)
        sentiment_label = self._score_to_label(sentiment_score)
        
        # Calculate detailed breakdowns
        emotion_scores = self._calculate_emotion_scores(processed_text)
        aspect_sentiments = self._calculate_aspect_sentiments(processed_text, nfl_context)
        ...
        context.sentiment_weights = self._get_sentiment_weights(processed_text, category)
    ...
    def _analyze_sentiment(
        ...
        positive_score = self._count_keywords(text, self.nfl_config.keywords.POSITIVE_PERFORMANCE)
        negative_score = self._count_keywords(text, self.nfl_config.keywords.NEGATIVE_PERFORMANCE)
        
        # Apply category weights
        category_weight = self.nfl_config.weights.CATEGORY_WEIGHTS.get(category.value, 1.0)
    ...
    def _calculate_aspect_sentiments(self, text: str, nfl_context: NFLContext) -> Dict[str, float]:
        """Calculate sentiment for different aspects"""
        aspects = {}
        
        # Performance aspect
        perf_positive = self._count_keywords(text, self.nfl_config.keywords.POSITIVE_PERFORMANCE)
        perf_negative = self._count_keywords(text, self.nfl_config.keywords.NEGATIVE_PERFORMANCE)
        if perf_positive + perf_negative > 0:
            aspects["performance"] = (perf_positive - perf_negative) / (perf_positive + perf_negative)
        ...
        # Trade aspect
        if nfl_context.trade_related:
            trade_score = self._count_keywords(text, self.nfl_config.keywords.TRADE_KEYWORDS)
    ...
    def _get_sentiment_weights(self, text: str, category: SentimentCategory) -> Dict[str, float]:
        """Get sentiment weights for the analysis"""
        weights = {}
        
        # Category weight
        weights["category"] = self.nfl_config.weights.CATEGORY_WEIGHTS.get(category.value, 1.0)
        
        # Keyword-based weights
        for keyword in self.nfl_config.get_all_keywords():
            if keyword in text:
                weights[keyword] = self.nfl_config.get_keyword_sentiment_weight(keyword, category.value)
