...
from ...models.mlops import (
    ModelMetadata, ModelRetrainingConfig, ExperimentRun, 
    ModelPerformanceMetrics, ModelType, ModelStatus, ExperimentStatus
)
...
class ModelRetrainingService:
    ...
    async def check_retraining_triggers(self, model_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not config or not config.enabled:
                return {"should_retrain": False, "reason": "No active retraining config"}
    ...
    async def monitor_model_performance(
        ...
        try:
            ...
            if trigger_check["should_retrain"]:
                logger.info(f"Retraining triggered for {model_name}: {trigger_check['triggers_activated']}")
    ...
    async def list_retraining_jobs(
        ...
        try:
            ...
            if self.db:
                ...
                cursor = self.db.experiment_runs.find(query).sort("started_at", -1).limit(limit)
    ...
    async def _execute_retraining(
        ...
        try:
            ...
            await self.wandb_service.log_metrics({
                ...
                "validation_samples": len(training_data[1]) if training_data and len(training_data) > 1 else 0
            })
            
            # Get base model for fine-tuning
            base_model = training_config.get("base_model", "cardiffnlp/twitter-roberta-base-sentiment-latest")
            
            # Fine-tune model
            model_metadata = await self.hf_service.fine_tune_model(
                base_model=base_model,
                training_data=training_data[0] if training_data else [],
                validation_data=training_data[1] if training_data and len(training_data) > 1 else None,
    ...
    async def _check_performance_trigger(
        ...
        try:
            ...
            recent_metrics = self.performance_history[model_name][-5:]  # Last 5 measurements
            if not recent_metrics:
                return False
            
            # Calculate average recent performance
            avg_accuracy = sum(m.accuracy or 0 for m in recent_metrics) / len(recent_metrics)
            avg_f1 = sum(m.f1_score or 0 for m in recent_metrics) / len(recent_metrics)
            
            # Check if below threshold
            return (avg_accuracy < config.performance_threshold or 
                    avg_f1 < config.performance_threshold)
    ...
    async def _check_data_volume_trigger(
        ...
        try:
            ...
            new_data_count = await self._get_new_data_count(
                model_name, 
                config.last_triggered or config.created_at
            )
    ...
    async def _get_training_data(self, model_name: str) -> Optional[Tuple[List[Dict], List[Dict]]]:
        ...
        try:
            ...
            if hasattr(self.hopsworks_service, 'get_training_data'):
                return await self.hopsworks_service.get_training_data(
                    feature_view_name=f"{model_name}_features",
                    version=1
                )
    ...
    def _should_deploy_retrained_model(self, evaluation_metrics: Dict[str, float]) -> bool:
    ...
    async def _deploy_retrained_model(
        ...
        try:
            ...
            logger.info(f"Deployed retrained model to staging: {deployment.deployment_id}")
    ...
    def _format_retraining_status(self, experiment_run: ExperimentRun) -> Dict[str, Any]:
        ...
        return {
            ...
            "completed_at": experiment_run.completed_at.isoformat() if experiment_run.completed_at else None,
    ...
    async def _load_baseline_metrics(self):
        ...
        if self.db:
            try:
                ...
                logger.info(f"Loaded baseline metrics for {len(self.baseline_metrics)} models")
            except Exception as e:
