...
class NFLSentimentEngine:
    ...
    async def analyze_with_context(
        ...
        try:
            ...
            result = await self._enhance_with_nfl_features(result, include_detailed_analysis)
    ...
    async def batch_analyze_with_context(
        ...
        try:
            ...
            chunks = [texts[i:i + max_concurrent] for i in range(0, len(texts), max_concurrent)]
    ...
    def calculate_confidence_score(
        ...
        # Combine factors
        total_confidence = base_confidence + keyword_confidence + context_confidence + length_factor
    ...
    def _extract_enhanced_nfl_context(
        ...
        nfl_context.injury_related = any(
            keyword in text_lower for keyword in self.nfl_config.keywords.INJURY_KEYWORDS
        )
        ...
        performance_indicators = ["yards", "touchdowns", "sacks", "interceptions", "rating", "qbr"]
    ...
    async def _enhance_with_nfl_features(
        ...
        if include_detailed_analysis:
            result.aspect_sentiments.update(
                self._calculate_nfl_aspect_sentiments(result.text, result.context.nfl_context)
            )
    ...
    def _calculate_nfl_aspect_sentiments(
        self, 
        text: str, 
        nfl_context: Optional[NFLContext]
    ) -> Dict[str, float]:
        ...
        # Performance aspect
        perf_positive = sum(1 for kw in self.nfl_config.keywords.POSITIVE_PERFORMANCE if kw in text_lower)
        perf_negative = sum(1 for kw in self.nfl_config.keywords.NEGATIVE_PERFORMANCE if kw in text_lower)
        
        if perf_positive + perf_negative > 0:
            aspects["performance"] = (perf_positive - perf_negative) / (perf_positive + perf_negative)
        
        # Coaching aspect
        coaching_mentions = sum(1 for kw in self.nfl_config.keywords.COACHING_KEYWORDS if kw in text_lower)
        ...
        # Fantasy aspect
        fantasy_mentions = sum(1 for kw in self.nfl_config.keywords.FANTASY_KEYWORDS if kw in text_lower)
